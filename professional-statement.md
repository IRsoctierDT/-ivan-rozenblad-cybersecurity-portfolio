...
- **[AI Agentic MCPscan](https://github.com/IRsoctierDT/ai-agentic-mcpscan)** — an open-source (Apache-2.0), offline-by-default security posture scanner for MCP servers and local AI-agent setups, published on PyPI as `ai-agentic-mcpscan`. It finds exposed servers, plaintext secrets, over-broad tool scopes, and unpinned packages across Claude, Cursor, Windsurf, and Cline configurations, and grades posture A–F with prioritized, redacted remediation guidance.
...
- **Ship MCPscan to 1.0** — SARIF output, a GitHub code-scanning action, more host adapters, and an opt-in fix mode — and grow it into a trusted tool for the AI-agent security niche while the industry still lacks one.
